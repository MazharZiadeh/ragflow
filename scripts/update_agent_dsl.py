...
This script loads the agent template JSON, properly serializes it,
and updates the user_canvas table using parameterized queries.

Usage:
    # From host (runs inside ragflow container):
    docker exec -it docker-ragflow-gpu-1 python /ragflow/scripts/update_agent_dsl.py

    # Inside container:
    python /ragflow/scripts/update_agent_dsl.py
"""
...
if os.path.exists("/ragflow"):
    TEMPLATE_PATH = Path("/ragflow/agent/templates/main0_agent.json")
    SCRIPT_DIR = Path("/ragflow/scripts")
else:
    TEMPLATE_PATH = Path(__file__).parent.parent / "agent/templates/main0_agent.json"
...
def get_db_connection():
    ...
    # Try to read from environment or container config
...
def load_template():
    """Load and parse the agent template JSON."""
...
def main():
    ...
    try:
        ...
        try:
            ...
            if current_dsl:
                backup_path = SCRIPT_DIR / "dsl_backup.json"
                with open(backup_path, "w") as f:
                    if isinstance(current_dsl, str):
                        f.write(current_dsl)
                    else:
                        json.dump(current_dsl, f, indent=2)
        ...
        print("Please restart the ragflow container to apply changes:")
        print("  docker compose restart ragflow-gpu")
        print("=" * 60)
